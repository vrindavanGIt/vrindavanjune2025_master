...
$('.item-name').keyup(debounce(function(){
    let $this = $(this);   
    let sss= $this.val().toLowerCase().replace(/\b[a-z]/g, function(letter) {
            return letter.toUpperCase();
        });

    $(this).val(sss);   
    let  str = $this.val().replace(/^[^A-Z0-9]+|[^A-Z0-9]+$|([^A-Z0-9]+)/gi, (x,y) => y ? '-' : '');
...
$(document).on('click','.remove-menu',function(){
    ...
    $(function() {
        ...
        $(document ).ready(function(){ 
            ...
            let categoryFiredata=$('#subcategory_id').attr('data-crrunt_url');
            if(category_id != ''){            
                let url = $('#category_id').attr('data-href');
                if((categoryFiredata == 'back.digital.item.edit') && (categoryFiredata == 'back.digital.item.create')  ){
                    console.log('asdkjfhdsakjfh');
                    getCategory(url,category_id);
                }
        ...
        function getSubCategory(url,subcategory_id){
            $.get(url+'?subcategory_id='+subcategory_id,function(data){
                ...
                $('#childcategory_id').html(view_html);
            })
        }
        ...
        function getCategory(url,category_id){
            $.get(url+'?category_id='+category_id,function(data){
                let response = data.data;
                let view_html = ``;
                let subcategoryOldValue = $('#subcategoryOldValue').val();
                $.each(response , function(key, value) {
                    view_html += `<option ${subcategoryOldValue==value.id?'selected':''} value="${value.id}">${value.name}</option>`;
                  });
                  let start = `<option value="">Select One</option>`;
